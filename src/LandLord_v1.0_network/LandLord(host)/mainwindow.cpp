...
void MainWindow::GameStart(){
    ...
    //洗牌信号发出
    sendmessage.clear();
    sendmessage="s"; //s开头表示洗牌信号
    for(int i=0;i<(int)P->perm.size();i++){
        if(P->perm[i]<10){
            sendmessage+=(QString("00")+QString::number(P->perm[i]));
        }
        else{
            if(P->perm[i]>=10 && P->perm[i]<=99){
                sendmessage+=(QString("0")+QString::number(P->perm[i]));
            }
            else
                sendmessage+=QString::number(P->perm[i]);
        }
    }
    //this->TCP_sendMesSocket->write(sendmessage.toLatin1());//没事给自己发了信号
    //给client发信号
    this->TCP_connectSocket1->write(sendmessage.toLatin1());
    this->TCP_connectSocket2->write(sendmessage.toLatin1());
    this->TCP_connectSocket3->write(sendmessage.toLatin1());
    //
    P->Allocation(); //发牌
    ...
    ui->Player2Connect->hide();
    ui->Player3Connect->hide();
    ui->Player4Connect->hide();
}
...
void MainWindow::on_ButtonConfirm_clicked()
{
    GameStart();
    Overflag=false;
    //QSound::play(".debug/SE/GameStart.wav");
    PrintinitCard();
    ...
    //CurrentPlayer=LandLordRoundCurrentPlayer;
    RefreshCardStatus();
...
void MainWindow::on_Chief3Score_clicked()
{
    ...
    sendmessage.clear();
    qDebug()<<"Clear Successful!";
    sendmessage="0"+QString::number(thisplayer)+"1"; //0开头表示叫分信号
    //this->TCP_sendMesSocket->write(sendmessage.toLatin1());//为了方便给自己发信号
    //自己是不能给自己发信息的！！！！
    HumanLandLordDecided(1);
    this->TCP_connectSocket1->write(sendmessage.toLatin1());
    this->TCP_connectSocket2->write(sendmessage.toLatin1());
    this->TCP_connectSocket3->write(sendmessage.toLatin1());
    ButtonClick01->play();
...
void MainWindow::MyGroupBox::labelStatusChange(){
    if(index>=P->Player[*CurrentPlayer].num){
        return;
    }
...
void MainWindow::InitPlay(){
    ...
    */
    ...
    qDebug()<<"P->Player[seat[0]].num="<<P->Player[seat[0]].num;
    qDebug()<<"P->Player[seat[1]].num="<<P->Player[seat[1]].num;
    qDebug()<<"P->Player[seat[2]].num="<<P->Player[seat[2]].num;
    qDebug()<<"P->Player[seat[3]].num="<<P->Player[seat[3]].num;
    PlayerTextShow();
    LandLordExhibit();
    InTimePlayedCard();
...
void MainWindow::on_Play_clicked()
{
    ...
    //this->TCP_sendMesSocket->write(sendmessage.toLatin1());//给自己发信号
    //自己是不能给自己发信号的！！！！
    this->TCP_connectSocket1->write(sendmessage.toLatin1());
    this->TCP_connectSocket2->write(sendmessage.toLatin1());
    this->TCP_connectSocket3->write(sendmessage.toLatin1());
...
void MainWindow::read_data(){
    ...
    if (s[0]=='a'){
        ...
        //mSocket->writeDatagram(mSocket->localAddress(),address,port);
    }
}
...
void MainWindow::slot_newconnect()
{
    if(TCP_server->hasPendingConnections())  //查询是否有新连接
    {
        if (num_player==0){
            ...
            ui->Player2Connect->setText(str2qstr(string("玩家2已连接!")));
            num_player++;
            ...
            /*
            sendmessage.clear();
            sendmessage="N"+QString::number(num_player); //N开头表示编号信号
            this->TCP_connectSocket1->write(sendmessage.toLatin1());
            */
            //发送结束
        } else {
            if (num_player==1){
                ...
                ui->Player3Connect->setText(str2qstr(string("玩家3已连接!")));
                num_player++;
                ...
                */
                ...
                if (num_player==2){
                    ...
                    ui->Player4Connect->setText(str2qstr(string("玩家4已连接!")));
                    num_player++;
...
void MainWindow::slot_sendmessage()
{
    ...
    if(TCP_connectSocket2 != nullptr && !sendMessage.isEmpty()) //确保有客户端连接，并且发送内容不为空
    {
        TCP_connectSocket2->write(sendMessage.toLatin1());   //发送消息到客户端
    }
    if(TCP_connectSocket3 != nullptr && !sendMessage.isEmpty()) //确保有客户端连接，并且发送内容不为空
    {
        TCP_connectSocket3->write(sendMessage.toLatin1());   //发送消息到客户端
    }
}
...
void MainWindow::slot_recvmessage()
{
    ...
    if(TCP_connectSocket1 != nullptr)
    {
        ...
        recvmessage += array;
        //qDebug()<<"TCP_connectSocket1: "<<sendmessage;
    }
    if(TCP_connectSocket2 != nullptr)
    {
        QByteArray array = TCP_connectSocket2->readAll();
        recvmessage += array;
        //qDebug()<<"TCP_connectSocket2: "<<sendmessage;
    }
    if(TCP_connectSocket3 != nullptr)
    {
        QByteArray array = TCP_connectSocket3->readAll();
        recvmessage += array;
        //qDebug()<<"TCP_connectSocket3: "<<sendmessage;
    }
    if(recvmessage[0]=='0'){//叫分信号
        //先直接将叫分信号传递到客户端
        //this->TCP_sendMesSocket->write(recvmessage.toLatin1());
        this->TCP_connectSocket1->write(recvmessage.toLatin1());
        this->TCP_connectSocket2->write(recvmessage.toLatin1());
        this->TCP_connectSocket3->write(recvmessage.toLatin1());
        int a=(recvmessage.mid(2,1)).toInt();
        HumanLandLordDecided(a);
        if(thisplayer==LandLordRoundCurrentPlayer){
            ui->Chief0Score->show();
            ui->Chief1Score->show();
            ui->Chief2Score->show();
            ui->Chief3Score->show();
    ...
    if(recvmessage[0]=='p'){//准备信号
        int temp=(recvmessage.mid(1,1)).toInt();
        prepared[temp]=1;
        //qDebug()<<temp;
        if(prepared[0]==1 && prepared[1]==1 && prepared[2]==1 && prepared[3]==1){
...
void MainWindow::slot_disconnect()
{
    if(TCP_connectSocket1 != nullptr)
    {
        ui->Player2Connect->setText("P1 logout!");
        TCP_connectSocket1->close(); //关闭客户端
        TCP_connectSocket1->deleteLater();
    }
    if(TCP_connectSocket2 != nullptr)
    {
        ui->Player3Connect->setText("P2 logout!");
        TCP_connectSocket2->close(); //关闭客户端
        TCP_connectSocket2->deleteLater();
    }
    if(TCP_connectSocket3 != nullptr)
    {
        ui->Player4Connect->setText("P3 logout!");
        TCP_connectSocket3->close(); //关闭客户端
        TCP_connectSocket3->deleteLater();
    }
}

void MainWindow::numsignalsend(){
    sendmessage.clear();
    sendmessage="N"+QString::number(1); //N开头表示编号信号
    this->TCP_connectSocket1->write(sendmessage.toLatin1());
    sendmessage.clear();
    sendmessage="N"+QString::number(2); //N开头表示编号信号
    this->TCP_connectSocket2->write(sendmessage.toLatin1());
    sendmessage.clear();
    sendmessage="N"+QString::number(3); //N开头表示编号信号
    this->TCP_connectSocket3->write(sendmessage.toLatin1());
}

void MainWindow::on_NumSend_clicked()
{
    numsignalsend();
}
