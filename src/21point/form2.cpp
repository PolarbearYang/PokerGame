    ...
    ui(new Ui::Form2)
{
    ...
    hit_btn->hide();
    stand_btn->hide();
    connect(stand_btn,SIGNAL(clicked()),this,SLOT(player2stay()));
    connect(hit_btn,SIGNAL(clicked()),this,SLOT(player2hit()));
    c1 = new client;
    c2 = new client;
    c3 = new client;
    c4 = new client;
    ...
    c4m->setParent(this);
    QPixmap pix;
    c2->setava(welcome::m_ava);
    c2->setname(welcome::m_name);
    switch (c2->c_ava) {
        case 1:
...
void Form2::slot_connected()
{
    ...
    if(this->isconnetion)
    {
        ...
        sendMessage1 = "N";
        sendMessage1 += c2->c_name;
        sendMessage1 += QString::number(c2->c_ava);
...
void Form2::player1hit(){
    ...
    for (int i = 0; i < card_num ; i++){
        c1_list[i]->move(30,(card_num-1-i)*100+200);
        c1_list[i]->show();
    }
...
void Form2::player2hit(){
    if (m_Deck.GetNum()>=1){
        ...
        for (int i = 0; i < card_num ; i++){
            c2_list[i]->move(535-(card_num-1-i)*100,500); //再显示所有手牌
            c2_list[i]->show();
        }
...
void Form2::player3hit(){
    ...
    for (int i = 0; i < card_num ; i++){
        c3_list[i]->move(880,(card_num-1-i)*100+200); //再显示所有手牌
        c3_list[i]->show();
    }
...
void Form2::player1stay(){
    hit_btn->show();
    stand_btn->show();
}
void Form2::player2stay(){
    if(this->isconnetion)
    {
...
void Form2::play(){
    ...
    h_list[1]->show();
    showPlayer1cards(p1Player);
    c1_list[0]->move(30,300);
    c1_list[1]->move(30,200);
    c1_list[0]->show();
    c1_list[1]->show();
    showPlayer2cards(p2Player);
    c2_list[0]->move(435,500);
    c2_list[1]->move(535,500);
    c2_list[0]->show();
    c2_list[1]->show();
    showPlayer3cards(p3Player);
    c3_list[0]->move(880,300);
    c3_list[1]->move(880,200);
    c3_list[0]->show();
    c3_list[1]->show();
    showPlayer4cards(p4Player);
    c4_list[0]->move(535,100);
    c4_list[1]->move(435,100);
    c4_list[0]->show();
    c4_list[1]->show();
    Player::p1_total = p1Player.GetTotal();
    Player::p2_total = p2Player.GetTotal();
...
void Form2::player4hit(){
    ...
    showPlayer4cards(p4Player);
    for (int i = 0; i < card_num ; i++){
        c4_list[i]->move(435+(card_num-1-i)*100, 100);
        c4_list[i]->show();
    }
...
void Form2::showPlayer1cards(Player &aGenericPlayer){
...
void Form2::showPlayer2cards(Player &aGenericPlayer){
    //int card_num = aGenericPlayer.GetNum();
    vector<card*>::const_iterator iter; //采取枚举的显示牌的方式（开始挨骂）
    for (iter = aGenericPlayer.m_cards.begin(); iter != aGenericPlayer.m_cards.end(); ++iter){
        QLabel *label = new QLabel;
...
void Form2::showPlayer3cards(Player &aGenericPlayer){
    //int card_num = aGenericPlayer.GetNum();
    vector<card*>::const_iterator iter; //采取枚举的显示牌的方式（开始挨骂）
    for (iter = aGenericPlayer.m_cards.begin(); iter != aGenericPlayer.m_cards.end(); ++iter){
        QLabel *label = new QLabel;
...
void Form2::showPlayer4cards(Player &aGenericPlayer){
    //int card_num = aGenericPlayer.GetNum();
    vector<card*>::const_iterator iter; //采取枚举的显示牌的方式（开始挨骂）
    for (iter = aGenericPlayer.m_cards.begin(); iter != aGenericPlayer.m_cards.end(); ++iter){
        QLabel *label = new QLabel;
...
void Form2::showHousecards(House &aGenericPlayer){
    //int card_num = aGenericPlayer.GetNum();
...
void Form2::cleanHousecards(){
    h_list.clear();
}

void Form2::cleanPlayer1cards(){
    c1_list.clear();
}

void Form2::cleanPlayer2cards(){
    c2_list.clear();
}

void Form2::cleanPlayer3cards(){
    c3_list.clear();
}

void Form2::cleanPlayer4cards(){
    c4_list.clear();
}

void Form2::sleep(unsigned int msec){
