    ...
    ui(new Ui::Form3)
{
    ...
    hit_btn->hide();
    stand_btn->hide();
    connect(stand_btn,SIGNAL(clicked()),this,SLOT(player3stay()));
    connect(hit_btn,SIGNAL(clicked()),this,SLOT(player3hit()));
    c1 = new client;
    c2 = new client;
    c3 = new client;
    c4 = new client;
    ...
    c4m->setParent(this);
    QPixmap pix;
    c3->setava(welcome::m_ava);
    c3->setname(welcome::m_name);
    switch (c3->c_ava) {
        case 1:
...
void Form3::slot_connected()
{
    ...
    if(this->isconnetion)
    {
        ...
        sendMessage1 = "N";
        sendMessage1 += c3->c_name;
        sendMessage1 += QString::number(c3->c_ava);
...
void Form3::slot_recvmessage()
{
    ...
    QString recvMessage = byteArray;
        if (recvMessage[0] == 'N'){
            QString c4name;
            int i = 1;
            for (i; i<recvMessage.length()-1; i++){
                if (recvMessage[i+1] != "#"){
...
void Form3::player1hit(){
    m_Deck.Deal(p1Player);
    int card_num = p1Player.GetNum();
    for (int i = 0; i < (card_num - 1); i++){
        c1_list[i]->hide();  //隐藏所有手牌
    }
    cleanPlayer1cards(); //清空记录手牌的全局变量
    showPlayer1cards(p1Player);
    for (int i = 0; i < card_num ; i++){
        c1_list[i]->move(435+(card_num-1-i)*100,100); //再显示所有手牌
        c1_list[i]->show();
    }
...
void Form3::player2hit(){
    ...
    for (int i = 0; i < card_num ; i++){
        c2_list[i]->move(30,(card_num-1-i)*100+200); //再显示所有手牌
        c2_list[i]->show();
    }
...
void Form3::player3hit(){
    if (m_Deck.GetNum()>=1){
        ...
        for (int i = 0; i < (card_num - 1); i++){
            c3_list[i]->hide();  //隐藏所有手牌
        }
        cleanPlayer3cards(); //清空记录手牌的全局变量
        showPlayer3cards(p3Player);
        for (int i = 0; i < card_num ; i++){
            c3_list[i]->move(535-(card_num-1-i)*100,500); //再显示所有手牌
            c3_list[i]->show();
        }
...
void Form3::player1stay(){
}
void Form3::player2stay(){
    hit_btn->show();
    stand_btn->show();
}
void Form3::player3stay(){
    if(this->isconnetion)
    {
...
void Form3::play(){
    ...
    h_list[1]->show();
    showPlayer1cards(p1Player);
    c1_list[0]->move(535,100);
    c1_list[1]->move(435,100);
    c1_list[0]->show();
    c1_list[1]->show();
    showPlayer2cards(p2Player);
    c2_list[0]->move(30,300);
    c2_list[1]->move(30,200);
    c2_list[0]->show();
    c2_list[1]->show();
    showPlayer3cards(p3Player);
    c3_list[0]->move(435,500);
    c3_list[1]->move(535,500);
    c3_list[0]->show();
    c3_list[1]->show();
    showPlayer4cards(p4Player);
    c4_list[0]->move(880,300);
    c4_list[1]->move(880,200);
    c4_list[0]->show();
    c4_list[1]->show();
    Player::p1_total = p1Player.GetTotal();
    Player::p2_total = p2Player.GetTotal();
...
void Form3::player4hit(){
    ...
    showPlayer4cards(p4Player);
    for (int i = 0; i < card_num ; i++){
        c4_list[i]->move(880, 200+(card_num-1-i)*100);
        c4_list[i]->show();
    }
...
void Form3::showPlayer1cards(Player &aGenericPlayer){
...
void Form3::showPlayer2cards(Player &aGenericPlayer){
    //int card_num = aGenericPlayer.GetNum();
    vector<card*>::const_iterator iter; //采取枚举的显示牌的方式（开始挨骂）
    for (iter = aGenericPlayer.m_cards.begin(); iter != aGenericPlayer.m_cards.end(); ++iter){
        QLabel *label = new QLabel;
...
void Form3::showPlayer3cards(Player &aGenericPlayer){
    //int card_num = aGenericPlayer.GetNum();
    vector<card*>::const_iterator iter; //采取枚举的显示牌的方式（开始挨骂）
    for (iter = aGenericPlayer.m_cards.begin(); iter != aGenericPlayer.m_cards.end(); ++iter){
        QLabel *label = new QLabel;
...
void Form3::showPlayer4cards(Player &aGenericPlayer){
    //int card_num = aGenericPlayer.GetNum();
    vector<card*>::const_iterator iter; //采取枚举的显示牌的方式（开始挨骂）
    for (iter = aGenericPlayer.m_cards.begin(); iter != aGenericPlayer.m_cards.end(); ++iter){
        QLabel *label = new QLabel;
...
void Form3::showHousecards(House &aGenericPlayer){
    //int card_num = aGenericPlayer.GetNum();
...
void Form3::cleanHousecards(){
    h_list.clear();
}

void Form3::cleanPlayer1cards(){
    c1_list.clear();
}

void Form3::cleanPlayer2cards(){
    c2_list.clear();
}

void Form3::cleanPlayer3cards(){
    c3_list.clear();
}

void Form3::cleanPlayer4cards(){
    c4_list.clear();
}

void Form3::sleep(unsigned int msec){
