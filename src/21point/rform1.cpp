...
#include "ui_rform1.h"
#include <welcome.h>
int rform1::num_player=0;
int rform1::all_num=1;
int rform1::pre_num=0;
rform1::rform1(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::rform1)
{
    ui->setupUi(this);
    ...
    if(TCP_server->listen(QHostAddress("0.0.0.0"),8888))
    {
...
void rform1::read_data(){
    ...
    if (s[0]=='f'){
        ip = "s";
        ip += QString::number(rform1::num_player);
        ip += QString::number(rform1::all_num);
        ...
        if(s[0]=='c'){
            ip = "c";
            ip += QString::number(rform1::pre_num);
            ip += QString::number(rform1::all_num);
            rform1::pre_num++;
            array=ip.toLatin1();
            mSocket->writeDatagram(array,address,port);
...
void rform1::player1stay(){
    r2round();
}
void rform1::player2stay(){

}
void rform1::player3stay(){
    hit_btn->show();
    stand_btn->show();
}
void rform1::play(){
...
void rform1::showPlayer1cards(Player &aGenericPlayer){
    vector<card*>::const_iterator iter;
...
void rform1::showPlayer2cards(Player &aGenericPlayer){
...
void rform1::showPlayer3cards(Player &aGenericPlayer){
    //int card_num = aGenericPlayer.GetNum();
    vector<card*>::const_iterator iter; //采取枚举的显示牌的方式（开始挨骂）
    for (iter = aGenericPlayer.m_cards.begin(); iter != aGenericPlayer.m_cards.end(); ++iter){
        QLabel *label = new QLabel;
...
void rform1::showPlayer4cards(Player &aGenericPlayer){
    //int card_num = aGenericPlayer.GetNum();
    vector<card*>::const_iterator iter; //采取枚举的显示牌的方式（开始挨骂）
    for (iter = aGenericPlayer.m_cards.begin(); iter != aGenericPlayer.m_cards.end(); ++iter){
        QLabel *label = new QLabel;
...
void rform1::showHousecards(House &aGenericPlayer){
    //int card_num = aGenericPlayer.GetNum();
    vector<card*>::const_iterator iter;
    for (iter = aGenericPlayer.m_cards.begin(); iter != aGenericPlayer.m_cards.end(); ++iter){
        QLabel *label = new QLabel;
...
void rform1::cleanHousecards(){
    h_list.clear();
}

void rform1::cleanPlayer1cards(){
    c1_list.clear();
}

void rform1::cleanPlayer2cards(){
    c2_list.clear();
}

void rform1::cleanPlayer3cards(){
    c3_list.clear();
}

void rform1::cleanPlayer4cards(){
    c4_list.clear();
}
void rform1::sleep(unsigned int msec){
...
void rform1::on_pushButton_clicked()
{
    if(TCP_connectSocket1 != nullptr)
    {
        ui->label->setText("P1 logout!");
