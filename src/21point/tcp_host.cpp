#include "tcp_host.h"
#include "ui_tcp_host.h"
#include <QMessageBox>


TCP_host::TCP_host(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::TCP_host)
{
    ui->setupUi(this);
    ...
    if(TCP_server->listen(QHostAddress::LocalHost,8888)) //判断listen是否成功，成功则继续执行，连接新接收信号槽
    {
        qDebug() << 1;
...
//发送消息的槽
void TCP_host::slot_sendmessage()
{
...
//接收消息的槽
void TCP_host::slot_recvmessage()
{
